...
namespace DLSSFrameGenEnabler_WinUI3.Services
{
    public class GameScanner
    {
        ...
        public static List<GameInfo> ScanSteamGames()
        {
            ...
            try
            {
                var steamPaths = new[] { @"C:\Program Files (x86)\Steam", @"D:\SteamLibrary", @"E:\SteamLibrary" };
        ...
        {
            return AnalyzeGameDirectory(gamePath);
        }
        ...
        // 分析游戏目录
        private static GameInfo? AnalyzeGameDirectory(string gamePath, string? overrideName = null)
        {
            ...
            try
            {
                ...
                if (!hasRealGameExe)
                {
                    string[] binDirs = { "bin", "Binaries", "Bin64", "Bin32", "win64", "Win64", "x64", "game\\bin\\win64" };
                    foreach (var binDir in binDirs)
                    {
                        var fullPath = Path.Combine(gamePath, binDir);
                        if (Directory.Exists(fullPath))
                        {
                            foreach (var f in Directory.GetFiles(fullPath, "*.exe", SearchOption.TopDirectoryOnly))
                            {
                                try
                                {
                                    if (new FileInfo(f).Length > 1024 * 1024) // 大于1MB
        ...
        private static string? FindGameExeDirectory(string gamePath)
        {
            ...
            // ========== 其他常见目录结构 ==========
            string[] commonSubDirs = { "Binaries", "Win64", "Win32", "x64", "x86", "game", "Game" };
        ...
        private static string FindGameRoot(string exeDir)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                dir = parent.FullName;
                // 如果找到steamapps/common或有明显游戏特征，停止
                if (dir.Contains("steamapps") || dir.Contains("Epic Games")) break;
            }
            return dir;
        }
