    ...
    public class FilePatcher
    {
        ...
        private static void CopyPatchFiles(string patchDir, string destDir, string backupDir, bool backupExisting = true, string[]? skipExtensions = null)
        {
            ...
            foreach (var fileName in fileNames)
            {
                ...
                bool fileExisted = File.Exists(destFile);
                if (backupExisting && fileExisted)
                {
                ...
                if (!fileExisted)
                {
                    RecordAddedFile(backupDir, destFile);
                }
            }
        }
        ...
        // 复制目录（保持结构）
        private static void CopyDirectory(string source, string dest, bool overwrite = true, bool includeSharedFiles = false, string? backupDir = null)
        {
            bool dirExisted = Directory.Exists(dest);
            Directory.CreateDirectory(dest);
            // 记录新增的目录
            if (!dirExisted && backupDir != null)
            {
                RecordAddedDir(backupDir, dest);
            }
            ...
            {
                CopyDirectory(dir, Path.Combine(dest, Path.GetFileName(dir)), overwrite, includeSharedFiles: false, backupDir);
            }
        }

        // 复制中文字体文件到游戏目录的Fonts子目录
        private static void CopyFontsFiles(string exeDir, string? backupDir = null)
        {
            try
            {
                ...
                bool dirExisted = Directory.Exists(fontsDstDir);
                if (!dirExisted)
                {
                ...
                if (!dirExisted && backupDir != null)
                {
                    RecordAddedDir(backupDir, fontsDstDir);
                }
                ...
                foreach (var fontFile in Directory.GetFiles(sharedFontsDir, "*", SearchOption.AllDirectories))
                {
                    ...
                    if (!string.IsNullOrEmpty(dstFontDir) && !Directory.Exists(dstFontDir))
                    {
                        ...
                        if (backupDir != null)
                        {
                            RecordAddedDir(backupDir, dstFontDir);
                        }
                    }
                    bool fileExisted = File.Exists(dstFontFile);
                    File.Copy(fontFile, dstFontFile, true);
                    // 记录新增的文件
                    if (!fileExisted && backupDir != null)
                    {
                        RecordAddedFile(backupDir, dstFontFile);
                    }
                }
            }
            catch { }
        ...
        // 记录新增的文件（安装时复制的文件，还原时直接删除）
        private static void RecordAddedFile(string backupDir, string filePath)
        {
            try
            {
                var recordFile = Path.Combine(backupDir, "added_files.txt");
                var existingEntries = File.Exists(recordFile)
                    ? File.ReadAllLines(recordFile).ToList()
                    : new List<string>();
                if (!existingEntries.Contains(filePath, StringComparer.OrdinalIgnoreCase))
                {
                    existingEntries.Add(filePath);
                    File.WriteAllLines(recordFile, existingEntries);
                }
            }
            catch { }
        }

        // 记录新增的目录（安装时创建的目录，还原时直接删除）
        private static void RecordAddedDir(string backupDir, string dirPath)
        {
            try
            {
                var recordFile = Path.Combine(backupDir, "added_dirs.txt");
                var existingEntries = File.Exists(recordFile)
                    ? File.ReadAllLines(recordFile).ToList()
                    : new List<string>();
                if (!existingEntries.Contains(dirPath, StringComparer.OrdinalIgnoreCase))
                {
                    existingEntries.Add(dirPath);
                    File.WriteAllLines(recordFile, existingEntries);
                }
            }
            catch { }
        ...
        // 加载新增的文件列表
        private static HashSet<string> LoadAddedFiles(string backupDir)
        {
        ...
        // 加载新增的目录列表
        private static HashSet<string> LoadAddedDirs(string backupDir)
        {
            var result = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            try
            {
        ...
        private static bool IsAddedPatchFile(string fileName)
        {
            ...
            // 注意：binkw64/bink2w64/xinput/winhttp/wininet等游戏可能原生自带，移到IsReplacablePatchFile
            var patchDlls = new[] { "dxgi.dll", "d3d9.dll", "d3d11.dll", "d3d12.dll",
                "version.dll", "dinput8.dll", "winmm.dll", "dsound.dll",
        ...
        private static bool IsReplacablePatchFile(string fileName)
        {
            ...
            // Bink视频库（很多游戏原生自带！）
            if (name == "binkw64.dll" || name == "bink2w64.dll") return true;
            // XInput库（有些游戏可能原生自带）
            if (name == "xinput1_3.dll" || name == "xinput1_4.dll") return true;
            // WinHTTP/WinINet（有些游戏可能原生自带）
            if (name == "winhttp.dll" || name == "wininet.dll") return true;
            return false;
        }
        ...
        public static bool InstallClassicFrameGen(GameInfo game)
        {
            try
            {
                ...
                else
                {
                    ...
                    bool fileExisted = File.Exists(dstPath);
                    File.Copy(dlssgSrc, dstPath, true);
                    if (!fileExisted) RecordAddedFile(backupDir, dstPath);
                }
                ...
                foreach (var file in new[] { "RTX40MFG.asi", "RTX40MFG_config.json", "version.dll" })
                {
                    ...
                    bool fileExisted = File.Exists(dst);
                    if (fileExisted) BackupFile(dst, backupDir);
                    File.Copy(src, dst, true);
                    if (!fileExisted) RecordAddedFile(backupDir, dst);
                }
        ...
        public static bool InstallAdvancedFrameGen(GameInfo game)
        {
            try
            {
                ...
                bool nvngxDlssExisted = File.Exists(nvngxDlss);
                if (nvngxDlssExisted) BackupFile(nvngxDlss, backupDir);
                var nvngxDlssSrc = ResolvePatchFile(GetPatchDir(@"40系原生多帧生成\排错"), "nvngx_dlss.dll");
                File.Copy(nvngxDlssSrc, nvngxDlss, true);
                if (!nvngxDlssExisted) RecordAddedFile(backupDir, nvngxDlss);

                // 复制version.dll
                var versionDll = Path.Combine(exeDir, "version.dll");
                bool versionDllExisted = File.Exists(versionDll);
                if (versionDllExisted) BackupFile(versionDll, backupDir);
                File.Copy(Path.Combine(GetPatchDir("40系原生多帧生成"), "version.dll"), versionDll, true);
                if (!versionDllExisted) RecordAddedFile(backupDir, versionDll);
                ...
                if (Directory.Exists(shadersSrc))
                    CopyDirectory(shadersSrc, shadersDst, backupDir: backupDir);

                // 复制中文字体文件
                CopyFontsFiles(exeDir, backupDir);
        ...
        public static bool InstallRTX20FrameGen(GameInfo game)
        {
            try
            {
                ...
                if (Directory.Exists(runtimeSrc))
                    CopyDirectory(runtimeSrc, runtimeDst, backupDir: backupDir);
        ...
        public static bool InstallREEngineFrameGen(GameInfo game)
        {
            try
            {
                ...
                bool dinput8Existed = File.Exists(dinput8);
                if (dinput8Existed) BackupFile(dinput8, backupDir);
                File.Copy(Path.Combine(GetPatchDir("RE框架"), "dinput8.dll"), dinput8, true);
                if (!dinput8Existed) RecordAddedFile(backupDir, dinput8);
        ...
        public static bool Install2077FrameGen(GameInfo game)
        {
            try
            {
                ...
                if (Directory.Exists(pluginsSrc))
                    CopyDirectory(pluginsSrc, pluginsDst, backupDir: backupDir);
        ...
        public static bool InstallCommonDLSS5(GameInfo game)
        {
            try
            {
                ...
                CopyDirectory(reshadeDir, exeDir, includeSharedFiles: true, backupDir: backupDir);
        ...
        public static bool InstallYanYunDLSS5(GameInfo game)
        {
            try
            {
                ...
                if (Directory.Exists(shadersSrc))
                    CopyDirectory(shadersSrc, Path.Combine(exeDir, "reshade-shaders"), backupDir: backupDir);
                ...
                if (Directory.Exists(streamlineSrc))
                {
                    ...
                    if (gameStreamline != null)
                        CopyDirectory(streamlineSrc, gameStreamline, includeSharedFiles: true, backupDir: backupDir);
                    else
                        CopyDirectory(streamlineSrc, streamlineDst, includeSharedFiles: true, backupDir: backupDir);
                }
        ...
        public static bool InstallDX9DLSS5_32(GameInfo game)
        {
            try
            {
                ...
                if (Directory.Exists(host64Src))
                    CopyDirectory(host64Src, Path.Combine(exeDir, "host64"), includeSharedFiles: true, backupDir: backupDir);
        ...
        public static (List<string> filesToDelete, List<string> filesToRestore, List<string> dirsToDelete) PreviewRestoreFrameGen(GameInfo game)
        {
            ...
            try
            {
                ...
                var backupPathMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

                // 加载安装前的目录快照
                var snapshotFiles = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                string snapshotRoot = searchRoot;
                ...
                foreach (var backupDir in backupDirs)
                {
                    // 加载目录快照
                    var snapshotFile = Path.Combine(backupDir, "pre_install_snapshot.txt");
                    if (File.Exists(snapshotFile) && !hasSnapshot)
                    {
                        var (root, files) = LoadDirectorySnapshot(snapshotFile);
                    ...
                    foreach (var file in Directory.GetFiles(backupDir))
                    {
                        ...
                        if (!string.IsNullOrEmpty(originalPath) && !backupPathMap.ContainsKey(fileName))
                        {
                            backupPathMap[fileName] = originalPath;
                        }
                        filesToRestore.Add(originalPath ?? Path.Combine(exeDir, fileName));
                    }
                }

                // 收集会被删除的文件（使用和实际还原完全一致的逻辑：快照验证 + IsAddedPatchFile）
                ...
                foreach (var gameFile in allGameFiles)
                {
                    ...
                    if (hasSnapshot)
                    {
                        try
                        {
                            if (gameFile.StartsWith(snapshotRoot, StringComparison.OrdinalIgnoreCase))
                            {
                                var relativePath = gameFile.Substring(snapshotRoot.Length).TrimStart('\\', '/');
                                if (snapshotFiles.Contains(relativePath))
                                {
                                    isNewFile = false;
                                }
                            }
                    ...
                    if (isNewFile && IsAddedPatchFile(fileName))
                    {
                        filesToDelete.Add(gameFile);
                    }
                }

                // 会被删除的目录（和实际还原完全一致，使用快照验证）
                ...
                foreach (var dirName in dirNames)
                {
                    try
                    {
                        ...
                        {
                            if (!d.Contains(BackupBaseDir) && Directory.Exists(d))
                            {
                                bool dirIsNew = true;
                                if (hasSnapshot)
                                {
                                    try
                                    {
                                        if (d.StartsWith(snapshotRoot, StringComparison.OrdinalIgnoreCase))
                                        {
                                            var relativePath = d.Substring(snapshotRoot.Length).TrimStart('\\', '/');
                                            if (snapshotFiles.Any(f => f.StartsWith(relativePath + "\\", StringComparison.OrdinalIgnoreCase)))
                                            {
                                                dirIsNew = false;
                                            }
                                        }
                                ...
                                if (dirIsNew) dirsToDelete.Add(d);
                            }
                        }
                    }
                    catch { }
        ...
        public static bool RestoreFrameGen(GameInfo game)
        {
            try
            {
                ...
                var backedUpFileNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                var backupFileMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
                var backupPathMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase); // 文件名 -> 原始完整路径

                // 加载安装前的目录快照（用于精准判断哪些文件是新增的）
                var snapshotFiles = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                string snapshotRoot = searchRoot;
                bool hasSnapshot = false;

                // 加载安装时记录的新增文件和目录列表（优先使用，最准确）
                ...
                if (Directory.Exists(gameBackupRoot))
                {
                    ...
                    foreach (var backupDir in backupDirs)
                    {
                        ...
                        // 加载安装时记录的新增文件和目录
                        foreach (var f in LoadAddedFiles(backupDir)) addedFiles.Add(f);
                        foreach (var d in LoadAddedDirs(backupDir)) addedDirs.Add(d);

                        foreach (var file in Directory.GetFiles(backupDir))
                        {
                            var fileName = Path.GetFileName(file);
                            if (fileName == "backup_manifest.txt" || fileName == "game_path.txt" || fileName == "pre_install_snapshot.txt" || fileName == "added_files.txt" || fileName == "added_dirs.txt") continue;
                            backedUpFileNames.Add(fileName);
                            if (!backupFileMap.ContainsKey(fileName))
                            {
                                backupFileMap[fileName] = file;
                ...
                // 第二步：优先删除安装时记录的新增文件（最准确，不依赖文件名匹配）
                int deletedCount = 0;
                foreach (var addedFile in addedFiles)
                {
                    try
                    {
                        if (File.Exists(addedFile) && !addedFile.Contains(BackupBaseDir))
                        {
                ...
                // 第三步：递归搜索游戏目录中所有文件，根据类型处理
                // - 备份目录中有同名文件 → 被替换的文件 → 恢复备份的原生文件（使用manifest记录的原始路径）
                // - 快照中不存在 + 匹配"明确添加的补丁文件"模式 → 新增的文件 → 删除（双重保险）
                // - 其他文件（游戏原生）→ 完全不动
                var allGameFiles = GetFilesWithDepthLimit(searchRoot, "*", 8);
                int restoredCount = 0;
                ...
                foreach (var gameFile in allGameFiles)
                {
                    ...
                    {
                        if (gameFile.Contains(BackupBaseDir)) continue;
                        // 已经在added_files中处理过的跳过
                        if (addedFiles.Contains(gameFile)) continue;

                        var fileName = Path.GetFileName(gameFile);

                        // 情况1：备份目录中有同名文件 → 被替换的文件 → 恢复备份的原生文件
                        // 优先使用manifest记录的原始路径，确保恢复到正确的位置
                        if (backedUpFileNames.Contains(fileName) && backupFileMap.TryGetValue(fileName, out var backupFilePath))
                        {
                            // 优先使用manifest记录的原始路径
                            string originalPath;
                            if (backupPathMap.TryGetValue(fileName, out var manifestPath) && File.Exists(manifestPath))
                            {
                                originalPath = manifestPath;
                            }
                            else
                            {
                                // 如果manifest中没有，就恢复当前找到的这个文件
                                originalPath = gameFile;
                            }
                        ...
                        // 情况2：判断是否是新增的补丁文件 → 删除（双重保险）
                        // 使用双重验证：
                        //   a) 如果有安装前快照，快照中不存在的文件才是新增的
                        //   b) 文件名必须匹配IsAddedPatchFile模式
                        bool isNewFile = true;
                        if (hasSnapshot)
                        {
                            // 计算相对于快照根目录的相对路径
                            try
                            {
                                if (gameFile.StartsWith(snapshotRoot, StringComparison.OrdinalIgnoreCase))
                                {
                                    ...
                                    if (snapshotFiles.Contains(relativePath))
                                    {
                                        isNewFile = false; // 快照中存在，说明是游戏原生文件，不删除
                                    }
                                }
                            }
                            catch { }
                ...
                // 第四步：优先删除安装时记录的新增目录（最准确）
                foreach (var addedDir in addedDirs)
                {
                    try
                    {
                        if (Directory.Exists(addedDir) && !addedDir.Contains(BackupBaseDir))
                        {
                            Directory.Delete(addedDir, true);
                        }
                    }
                    catch { }
                }

                // 第五步：删除补丁安装时创建的目录（双重保险）
                // 注意：使用快照验证，只有快照中不存在的目录才删除（不会误删游戏原生目录）
                var dirsToDelete = new[] { "reshade-shaders", "runtime", "host64", "Fonts" };
                foreach (var dirName in dirsToDelete)
                {
                    try
                    {
                        ...
                        foreach (var foundDir in foundDirs)
                        {
                            ...
                            {
                                // 如果有快照，检查这个目录是否在快照中存在
                                bool dirIsNew = true;
                                if (hasSnapshot)
                                {
                                    try
                                    {
                                        if (foundDir.StartsWith(snapshotRoot, StringComparison.OrdinalIgnoreCase))
                                        {
                                            var relativePath = foundDir.Substring(snapshotRoot.Length).TrimStart('\\', '/');
                                            // 检查快照中是否有这个目录下的任何文件
                                            if (snapshotFiles.Any(f => f.StartsWith(relativePath + "\\", StringComparison.OrdinalIgnoreCase)))
                                            {
                                                dirIsNew = false;
        ...
        public static bool RestoreDLSS5(GameInfo game)
        {
            try
            {
                ...
                // 第五步：删除补丁安装时创建的目录（双重保险）
                // 注意：使用快照验证，只有快照中不存在的目录才删除（不会误删游戏原生目录）
                var dirsToDelete = new[] { "reshade-shaders", "host64", "runtime", "Fonts" };
                foreach (var dirName in dirsToDelete)
                {
                    try
                    {
